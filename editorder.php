...
if (isset($_POST['btneditorder'])) {
    ...
    foreach($row_invoice_details as $item_invoice_details){
        ...
        // $get_pstock = $pdo->prepare("select productstock from tbl_product where id=".$item_invoice_details['product_id']);

        // $get_pstock->execute();
        // print_r($get_pstock);
        $myproductstock = $arr_stock[0] + $item_invoice_details['quantity'];
        
        $updateproductstock = $pdo->prepare("update tbl_product set (productstock=:pstock) where id='".$item_invoice_details['product_id']."'");
        
        $updateproductstock->bindParam(":pstock", $myproductstock);
